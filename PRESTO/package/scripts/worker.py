...
class Worker(Script):
  ...
  def configure(self, env):
    ...
    # write out worker.properties
    File(format("{params.conf_dir}/worker.properties"), content=params.worker_properties_content,
         owner=params.presto_user, group=params.presto_group)
  ...
  def stop(self, env):
    ...
    self.create_presto_log_dir(env)
    self.chown_presto_pid_dir(env)
    Execute(params.presto_dir + '/bin/launcher ' + params.launcher_worker_options + ' stop >> ' + params.presto_log_file, user=params.presto_user)
  ...
  def start(self, env):
    ...
    Execute(params.presto_dir + '/bin/launcher ' + params.launcher_worker_options + ' start >> ' + params.presto_log_file, user=params.presto_user)
    
    pidfile = glob.glob(os.path.join(status_params.presto_pid_dir, 'worker.pid'))[0]
  ...
  def status(self, env):
    ...
    try:
        pid_file = glob.glob(status_params.presto_pid_dir + '/worker.pid')[0]
    except IndexError:
        pid_file = ''
